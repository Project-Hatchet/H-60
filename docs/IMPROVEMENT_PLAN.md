...
> **STATUS: COMPLETE (2026-09-04).** Every Phase 0 item below plus the addendum's Phase 0-grade findings shipped across the fix-branch PRs and landed in **Stable 0.7.9** (Workshop + GitHub release v0.7.9). The dev-test loop that carried them (the `push_dev.py` devbuild pipeline, automated version stamping) is now standing infrastructure — see the Phase 3 note. Next: Phase 1.
...
6. Add **`vtx_S70i`** (renamed from `vtx_S70M` 2026-09-07, declared only in uh60_config) to the spawn-test matrix; the plan omits it entirely.
...
**Standing infrastructure from the 0.7.9 cycle (2026-08/09, PRs #585/#587/#600):** a devbuild pipeline now exists — `push_dev.py` builds and uploads Workshop Dev builds from a branch with squashed PR merges, queued (not cancelled) concurrent rebuilds, and automatic dev version stamping. That partially delivers 3.1's "version is generated, never hand-edited" for dev builds; the HEMTT migration must absorb this pipeline (same behaviors on `hemtt` instead of SCons), not sidestep it. Stable-build gotcha recorded during the 0.7.9 ship: a stable build produced via the dev pipeline must swap in the repo-root Stable `mod.cpp` (the pipeline stamps the Dev-branded one).
...
- **FMS input model (decided 2026-09-04, Riverman).** Cockpit interaction points cover only the FMS's **fixed function keys** (page/function row, CLR/ENT/arrows) — not per-letter/number buttons, which would be painful to click. Alphanumeric entry instead uses a **typed keyboard scratchpad** on the AH-64D pattern: their `fza_ah64_ku` addon installs a `CBA_fnc_addDisplayHandler ["KeyDown", …]` only while entry is active, swallows handled keys by returning `true`, maps DIK codes via `BIS_fnc_keyCode`, keeps the buffer/state as per-vehicle `setVariable`s, commits on Enter and cancels on Esc, and exposes a prompt API with per-type input **checkers** (free text / integer / grid / code) that pages use to request input. Port the shape, not the code verbatim (their numpad char map has a copy-paste bug mapping every digit to `DIK_NUMPAD0`). Ride with the Phase 5 FMS/waypoint work (#371); PR #591's function-row interaction points are the cockpit-side foundation. The MH-47G's CAAS MFD suite comes over to the MH-60M variants after the H-47 mod releases; the other variants keep the legacy MFDs (two cockpit generations coexist). Slot architecture settled: `userMFDvalue` capacity is per vehicle class (no pack split needed); Arma 2.20 raised the engine cap to 256 via `mfdMaxUserValues` (we currently set 100 in one place, `uh60_anvishud`, and use slots 0–97). Raise to 256; legacy/common writers keep 0–97 untouched on all variants; **CAAS gets the clean region 100–255**, shipped as a separate `uh60_caas` addon consumed by the MH-60M configs. The 5.5 slot ledger covers both regions. Diligence when this lands: in-game sanity test of a slot ≥100 at `mfdMaxUserValues=256`, confirm no per-frame cost. Secondary option verified in the AH-64D reference (`fza_ah64_missionplanner`): CT_WEBBROWSER popup displays for interactive panels (FMS/CDU, TAC map) — popup-only (cannot project onto cockpit glass), no slot limits, HTML reviewed like SQF.

**5.6 MH-60S (Knighthawk) rehabilitation.** The MH-60S variants are currently hidden from players — they broke after the 0.6.0 Army-model rework and are awaiting a dedicated pass. Their configs (including legitimate FLIR fits) are still in the tree and load-bearing for the class hierarchy, so the restructure phases must carry them along; schedule the actual rehabilitation after Phase 1 (deterministic base class) and 5.1 (systems consolidation), since both change the ground the Knighthawks stand on. Concrete inputs secured 2026-08-23:

- **TCG Navy asset intake (permission granted by the authors, Myth & Houdini).** Their Workshop reskin carries five livery textures (hull/misc/tail/markings + LASS) whose hidden-selection slots (16-19) verifiably match our `HIDDENSELECTION_HULL/MISC/TAIL/MARKINGS` layout on the real `vtx_MH60S` classes — they integrate as `CfgTextureSources` garage liveries with the authors credited, exactly like the existing livery entries. Decision: **no interim release** — the assets land as part of the rehab, done right, not as a stopgap on the Army airframe.
- **Hangman FLIR mount.** Their shipped `hangman.p3d` is ODOL-only; request the MLOD source from the authors, then the modeler merges it into the MH-60S geometry behind a `Hangman_show` animation source (the `ESSS_show`/`LASS_show` pattern), positioned to match the `pilotCamera` origin so the visible ball is the sensor the FLIR camera looks out of. Do NOT copy their runtime attach approach: their XEH InitPost `createVehicle`+`attachTo` script runs on every machine, so it duplicates the prop once per connected client in MP.
- **Their `initPhase` preset list is the S-configuration recipe.** The TCG classes (built on `vtx_MH60M` as a workaround for our hidden classes — not to be merged) encode a field-tested "what an MH-60S looks like" table: radar hidden, minigun sights hidden, ERFS off, MH-60M exterior parts off, MAWS tubes shown, ESSS on. Apply that recipe to our real classes during the rehab.
- **GAU-21 model replacement.** Replace our GAU-21 model with the newer, higher-fidelity GAU-21 asset already available to the team (source and contact noted internally); schedule it with the MH-60S work since the Knighthawk's `GAU21L` fits are the main consumer, and carry the change through to the MH-60M DAP mounts for consistency.
- **Attribution prerequisite:** get the authors' permission in writing (a Discord screenshot in the repo record suffices) before the assets land; credit them in the texture-source author fields and CfgPatches.

**Weapons pass (backlog, running list — added 2026-09-04).** Items queued for a dedicated Phase 5 weapons sweep: (1) **Hold-to-lase becomes the DEFAULT laser-designator behavior** (Riverman, 2026-08-31): once the separate hold keybind shipped in 0.7.9 (PR #584, modeled on the IZLID hold pattern) has soaked, the primary "Fire Laser" bind converts to press-to-lase/release-to-stop via `onActivate`/`onDeactivate`; decide then whether the toggle survives as a secondary bind (removal is a keybind-behavior change worth a changelog callout). (2) The possible master-arm-safed stuck-lase path noted on PR #584 during 0.7.9 testing. (3) The GAU-21 model replacement rides with 5.6 (above) but its DAP-mount carry-through belongs here.
...
All 17 open issues on `github.com/Project-Hatchet/H-60` (checked 2026-08-16) map onto the plan — none contradict it, several independently confirm its diagnoses. *(Refresh 2026-09-04: the tracker now carries per-phase milestones mirroring this plan; rows below marked SHIPPED closed during the Phase 0 / 0.7.9 cycle.)*
...
| **#442** | Server crashes from network overload; load ramps when a pilot boards; 10 H-60s killed a 50-player server | The **Phase 0.6** part (event handlers leak on every boarding) **SHIPPED** (#577, in 0.7.9). Remainder: **Phase 5.1** (deadband publishers, change-only broadcast, locality tiers) + the `spawn`-per-frame in `uh60_misc/fnc_perFrame.sqf`. |
| **#436** | `Error Nullteiler` in `fnc_getLaserCode` + frame drops | Was **Phase 0.11** — **SHIPPED** (Phase 0, Stable 0.7.9). |
| **#547** | FD mode increase/decrease keybinds do nothing | Was **Phase 0.12** — **SHIPPED** (Phase 0, Stable 0.7.9; framework-side knob fix landed in the Interaction-Framework repo). |
| **#498** (confirmed, "unable to fix") | AFM helicopter desyncs/dances on the ground for non-owner players | Flight-model locality — belongs with **Phase 4.5** (sfmplus boundary). **Root-cause hypothesis (2026-09-03, from FCD's four-bird dedicated-server test):** the parking brake is applied via `setBrakesRTD` (uh60_engine `fnc_wheelBrakes`) remote-exec'd **once** to wherever the vehicle is local; on ownership transfer (pilot exit/handover) the new owner's RotorLib never receives the brake state, so the simulations disagree at ground contact — explains the handover-triggered, ground-only, AFM-only signature that survived the #577 EH-leak fix. Fix shape: store intended brake state on the vehicle and re-assert on a "Local" EH; audit uh60_engine for other one-shot local-effect state while there. The MP repro rig remains the verification prerequisite; MH-47G's RTD/PhysX suspension notes (`physx.hpp`) are the best available reference material. |
...
| **#557** | Cabin crew can trigger countermeasures/autohover keybinds | **SHIPPED** — autohover blocked from rear-crew/cabin seats (#589, in 0.7.9); issue closed. The general seat-gating pattern still lands with **Phase 4** keybind consolidation (and the planned framework countermeasure-policy migration in Phase 5). |
...
| **#419** | First waypoint in list produces no TAC map marker | **Tabled to Phase 5 (Riverman, 2026-08-24)** after three fix attempts exposed structural issues — the FMS mixes the engine's start-waypoint-counting index system with the raw `waypoints` array, and the TAC MFD element visibility conditions cannot actually suppress drawing (marker-hiding must be done by geometry). Rides with the **#371** waypoint-system rework / **5.5** MFD pass; the verified engine findings are preserved in the project record. |
...
| **#522** | Fixed-forward IZLID laser on a keybind | **SHIPPED** — press-and-hold IZLID keybind (#581, in 0.7.9) on top of the two IZLID beam-function bug fixes (#579); issue closed. The MH-47G synced-variable on/off pattern remains the reference if the CBA-event toggle ever misbehaves in MP. |
...
### Fix immediately (Phase 0 tail — live defects) — *SHIPPED with Phase 0 (Stable 0.7.9): see PRs #570 (dev scripts/TGA packing), #572 (config typos), #578 (remote-exec whitelist) and siblings*
...
**Immediate (Phase 0-grade)** — *SHIPPED (PR #579, 2026-08-24):*
