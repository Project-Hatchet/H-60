...
**5.8 Take seat-behavior policy back from the Interaction-Framework (coordinated cross-repo change, no urgency).** The framework's `util` addon auto-installs global input hooks on every client: countermeasures from turret seats (`launchCM`), turret-optics zoom from internal view (`zoomIn`/`zoomOut`), and the in-vehicle salute. The first two are per-airframe gameplay policy living at framework level — every mod that adopts the framework inherits them unasked, and airframe-specific rulings (like the "no CMs from FFV seats" restriction, framework commit `0c3c930`) get made globally with seat heuristics instead of per-seat config. Decided 2026-08-27; current behavior works and isn't hurting anything, so this rides with Phase 5 rather than jumping the queue.

- **Framework side:** keep `hct_util_fnc_deployCountermeasures` and `hct_util_fnc_zoomTurret` as opt-in library functions (the CM function fires the airframe's shared dispenser pool, which per-turret vanilla `CMFlareLauncher` weapons could not replicate); delete the global `launchCM`/`zoomIn`/`zoomOut` handlers from `util/inputAction.inc.sqf`. This is a breaking change for framework consumers — the framework release must be coordinated with the H-60 module landing and say so in its changelog. The salute hook stays framework-global: it is universal QoL, not airframe policy, and its `CfgMoves` gesture asset has to live in a config addon anyway.
- **H-60 side:** small modules placed in the `hct` seat classes for exactly the seats that should have each behavior — CM dispense on the crew-chief/door-gunner `hct_turret_X` classes, zoom capture on the MFD/PiP seats — with `setup` registering the user-action handler and `shutDown` removing it. Seat selection then happens by config placement, replacing the framework's FFV/cargo filtering heuristics entirely. Nice-to-have while in there: a cockpit dispenser-panel interaction for the crew chief's flare punch, which is the framework's native idiom anyway.
- **Timing:** independently landable like the rest of Phase 5; natural to do alongside 5.1 since the new modules should declare their locality tier under the same rules. The acute on-foot error class in these hooks was already fixed framework-side (framework PR #62), which is what removes the urgency.
