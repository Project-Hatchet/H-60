...
private _camPosASL = htf_vehicle modelToWorldVisualWorld vtx_uh60_flir_camPos;
...
if (_isTracking) then {
  if (vtx_uh60_flir_isInScriptedCamera) then { // Fix laser target wandering off
    private _laserTarget = laserTarget htf_vehicle;
...
//drawLine3D [ASLToAGL _camPosASL, _camPosASL vectorAdd (htf_vehicle vectorModelToWorldVisual _dir), [1,0,0,1]];
//drawLine3D [ASLToAGL _camPosASL, _camPosASL vectorAdd ((htf_vehicle vectorModelToWorldVisual getPilotCameraDirection htf_vehicle) vectorMultiply 1000), [0,0,1,1]];
