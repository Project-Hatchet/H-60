...
if (vtx_uh60_ui_helpEnabled) then {
    private _hasDoorgun = [_vehicle, "doorguns"] call htf_core_fnc_hasModule;
    if (!_hasDoorgun) then {
        drawIcon3D [
...
if (vtx_uh60m_trackIR_interaction_cursor) then {
    ...
    htf_interaction_cursorPos params ["_curX", "_curY"];
    if (abs _x < 0.02) then { _x = 0; };
    if (abs _y < 0.02) then { _y = 0; };
    htf_interaction_cursorPos = [
...
CTRL(4001120) ctrlSetPosition [
    (htf_interaction_cursorPos # 0) - 0.02,
    (htf_interaction_cursorPos # 1) - 0.02
];
