...
private _outputForce = -1 * ([_vehicle, "hdg", _frameTime, 0, _headingDiff] call htf_util_fnc_pidRun);
...
if (abs _headingDiff < 2) then {
	...
	_desiredBank = ([_vehicle, "drift", _frameTime, 0, _drift] call htf_util_fnc_pidRun);
	//systemChat format ["CORRECTING DRIF %1 WITH %2", _drift, _desiredBank];
};

(_vehicle call BIS_fnc_getPitchBank) params ["_pitch", "_bank"];
private _bankForce = -1 * ([_vehicle, "roll", _frameTime, _desiredBank max -30 min 30, _bank] call htf_util_fnc_pidRun);
