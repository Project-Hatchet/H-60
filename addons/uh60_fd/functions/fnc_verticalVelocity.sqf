...
if (difficultyEnabledRTD) then {
	...
	private _output = [_vehicle, "collective", _frameTime, _verticalVelocityGoal, _verticalVelocity] call htf_util_fnc_pidRun;
	_vehicle setActualCollectiveRTD (_output min 1 max 0);
} else {
	if (vtx_uh60_rotorRPM < 0.04) exitWith {};
	private _output = [_vehicle, "collectiveSFM", _frameTime, _verticalVelocityGoal, _verticalVelocity] call htf_util_fnc_pidRun;
